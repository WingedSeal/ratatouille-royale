...
class InspectHistory(Page):
    def __init__(
        ...
        self.map_panel_element: ElementWrapper | None = None
    ...
    def _draw_map_with_movement(self, payload: MoveHistoryPayload) -> None:
        ...
        from_coords = tuple(map(int, from_str.split(",")))
        to_coords = tuple(map(int, to_str.split(",")))
        ...
        # Calculate hex size and grid bounds - make it larger and more visible
        hex_radius = 30
        padding = 10
        
        # Draw grid of hexagons
        self._draw_hexagon_grid(self.map_surface, hex_radius, padding, from_coords, to_coords)
        
        # Store reference to panel for later drawing
        self.map_panel_element = map_panel
    
    def _draw_hexagon_grid(self, surface: pygame.Surface, hex_radius: int, padding: int, 
                          from_coords: tuple, to_coords: tuple) -> None:
        """Draw a grid of square tiles showing the actual movement area at real scale."""
        import math
        
        # Show tiles in a reasonable range around the movement
        min_q = min(from_coords[0], to_coords[0]) - 2
        max_q = max(from_coords[0], to_coords[0]) + 2
        min_r = min(from_coords[1], to_coords[1]) - 2
        max_r = max(from_coords[1], to_coords[1]) + 2
        
        grid_width = max_q - min_q + 1
        grid_height = max_r - min_r + 1
        
        # Calculate tile size based on hex_radius (convert to square)
        tile_size = hex_radius * 2
        ...
        if distance > 0:
            ...
            # Draw arrow line
            pygame.draw.line(surface, (255, 200, 100), (arrow_start_x, arrow_start_y), 
    ...
    def _draw_square(self, surface: pygame.Surface, x: float, y: float, size: float, color: tuple, border_width: int = 1) -> None:
